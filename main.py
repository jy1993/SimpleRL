...
from transformers import Qwen2Tokenizer, Qwen2ForCausalLM, Qwen2_5_VLForConditionalGeneration, AutoProcessor, get_linear_schedule_with_warmup, get_constant_schedule_with_warmup, StoppingCriteriaList
...
class Trainer(object):
	...
	def __init__(self, args):
		...
		elif self.args.task_type in ['logic', 'math']:
	...
	def save_model(self, global_step):
		...
		save_file(sd, os.path.join(self.args.output_dir, 'checkpoint_%s' % global_step, 'model.safetensors'))
...
class TextTrainer(Trainer):
	...
	def eval_model(self):
		...
		for sample in self.eval_data:
			prompt = {'prompt': self.apply_chat_template(self.system_prompt, self.prompt_prefix + sample['query'])}
